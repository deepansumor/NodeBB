define("forum/auditAgent/store-summary", ["api"], function (api) {
	...
	storeSummary.init = async function () {
		...
		const humanSummaryRaw = data.humanSummary || "";
		const humanSummary = simpleMarkdown(humanSummaryRaw);
		...
		scores.forEach((item) => {
			...
			circle.className = `score-circle ${
				item.score >= 7 ? "circle-red" : "circle-yellow"
			}`;
      ...
      <div class="card shadow mb-4">
        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-md-8">
              <div class="mb-3 fs-6 bg-light p-3 rounded border">
                ${humanSummary}
              </div>
            </div>  <!-- Close col-md-8 here -->

            <div class="col-md-4">
              <div class="border rounded shadow-sm p-3 bg-white h-100">
                <h6 class="mb-3 text-muted">
                  <i class="bi bi-bar-chart-line-fill me-1 text-warning"></i>Performance Metrics
                </h6>
  ...
  <div class="text-center px-3 py-2 bg-light rounded-3" style="width: 120px;">
    <p class="text-muted small mb-1">OVERALL SCORE</p>
        <h4 class="fw-bold ${getScoreClass(
					p.overallScore
				)} mb-0">${p.overallScore.toFixed(1)}</h4>
  ...
  <div style="width: 300px;">
    <h6 class="fw-semibold mb-1" title="${p.title}">${truncateWords(
						p.title
					)}</h6>
  ...
  <div class="d-flex flex-wrap gap-2" style="flex: 1 1 auto; min-width: 360px;">
    ${Object.entries(p.categoryScores)
			.map(
				([cat, score]) => `
      <div class="border rounded-3 p-2 text-center" style="width: 100px;">
        <div class="fw-bold ${getScoreClass(score)}">${
        ...
        <div class="progress bg-light mt-1" style="height: 5px;">
          <div class="progress-bar ${getScoreClass(score)}" style="width: ${
					score * 10
				}%"></div>
        </div>
      </div>
...
`
		...
		document
			.getElementById("performance-filter")
