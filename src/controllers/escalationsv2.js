...
//                                                                                     // req.uid
//     console.log("rootCids from the controller --->", await privileges.categories.get(288,5));
...
//     res.render("automate/escalationsv2", data);
// };
...
escalationController.get = async function (req, res, next) {
    ...
    // added for previllages 

    const categoryPrivileges = await Promise.all(
        categoryData.map(cat => privileges.categories.get(cat.cid, req.uid))
    );

    // Attach privileges to each category object
    categoryData.forEach((cat, i) => {
        cat.privileges = categoryPrivileges[i];
    });
